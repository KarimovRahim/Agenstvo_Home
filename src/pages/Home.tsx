    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="bg-slate-900 text-white py-20 sm:py-32 px-4 relative flex flex-col justify-center min-h-[500px]">
        ...
        <div className="relative max-w-7xl mx-auto text-center w-full">
          ...
          >
            Премиальная недвижимость <br className="hidden sm:block" /> для вашей жизни
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <motion.div 
            ...
            className="max-w-4xl mx-auto flex flex-col sm:flex-row gap-3 bg-white/10 backdrop-blur-xl p-3 sm:rounded-[2rem] rounded-3xl border border-white/20 shadow-2xl mx-4 sm:mx-auto"
          >
            <div className="flex-1 flex items-center bg-white rounded-2xl sm:rounded-[1.5rem] px-5 sm:px-6 h-14 sm:h-16">
              ...
              <input 
                ...
                className="w-full bg-transparent border-none focus:ring-0 text-slate-900 placeholder-slate-400 px-4 h-full outline-none font-medium text-base sm:text-lg"
              />
            </div>
            <button className="h-14 sm:h-16 bg-slate-900 text-white px-8 sm:px-12 rounded-2xl sm:rounded-[1.5rem] font-bold text-base sm:text-lg hover:bg-slate-800 transition-colors shadow-sm shrink-0 whitespace-nowrap active:scale-[0.98]">
              Найти объект
            </button>
          </motion.div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12 sm:mb-16">
          ...
          <div className="shrink-0 text-sm font-semibold text-slate-500 bg-white px-6 py-3 rounded-2xl border border-slate-200 shadow-sm">
            Найдено: <span className="text-slate-900 ml-1">{filteredProperties.length}</span>
          </div>
        </div>
        ...
        {filteredProperties.length === 0 && (
          ...
          >
            ...
            <h3 className="text-2xl font-serif font-bold text-slate-900 mb-3">Ничего не найдено</h3>
            <p className="text-slate-500 font-medium max-w-md mx-auto leading-relaxed">
              К сожалению, по данным критериям объектов нет. Измените параметры фильтрации или обновите условия поиска.
            </p>
          </motion.div>
        )}
      </div>
